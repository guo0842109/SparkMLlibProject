...
def sampleInteractions(item_id,users_with_rating,n):
    ...
    if len(users_with_rating) > n:
        #print(random.sample(list(users_with_rating),n))
        return item_id, random.sample(list(users_with_rating),n)
    else:
...
def nearestNeighbors(user,users_and_sims,n):
    '''
        选出相似度最高的N个邻居
    '''
    list(users_and_sims).sort(key=lambda x: x[1][0],reverse=True)
    #return user, users_and_sims[:n]
    #print(users_and_sims)
    if len(users_and_sims) >= n:
...
# User based 
# user -> user -> item
# compute the similarity of user - user
if __name__ == "__main__":
    ...
    '''
        下面操作的输出：获得2个用户所有的item-item对得分组合:
    ...
    '''
        ...
        对每个用户找到其相近用户列表(rdd中的每个项，根据相似度计算出来)。然后在相近用户中给每个用户喜欢的
        物品根据rating和相似度打出综合得分。
    '''
