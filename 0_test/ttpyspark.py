...
lines = sc.textFile("../0_test/test.txt") # hdfs://
# lines = sc.textFile("/Users/horace/work/pythonworkspace/git-demo/SparkMLlibProject/0_test/test.txt") # hdfs://

# file:///usr/local/spark/README.md
tmp = lines.flatMap(lambda x:x.split(' ')).map(lambda x:(x,1))
counts = tmp.reduceByKey(add_2)
output = counts.collect() # count / take
